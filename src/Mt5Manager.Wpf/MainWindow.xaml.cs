...
public partial class MainWindow:Window
{
 ...
 readonly DispatcherTimer backgroundRefresh=new(){Interval=TimeSpan.FromSeconds(15)};
 public MainWindow(MainViewModel viewModel,ITerminalStorageInspector inspector,TerminalOperationCoordinator coordinator){InitializeComponent();this.viewModel=viewModel;this.inspector=inspector;this.coordinator=coordinator;DataContext=viewModel;backgroundRefresh.Tick+=(_,_)=>_=viewModel.RefreshStatesAsync();}
 async void Window_Loaded(object sender,RoutedEventArgs e){await viewModel.RefreshAsync();backgroundRefresh.Start();}
 void Window_Closed(object? sender,EventArgs e){backgroundRefresh.Stop();viewModel.CancelRefresh();}
