...
{
 readonly MainViewModel viewModel; readonly ITerminalStorageInspector inspector; readonly TerminalOperationCoordinator coordinator; readonly Func<TelegramSettingsDialog>? createTelegramDialog;
 ...
 public MainWindow(MainViewModel viewModel,ITerminalStorageInspector inspector,TerminalOperationCoordinator coordinator,Func<TelegramSettingsDialog>? createTelegramDialog=null){InitializeComponent();this.viewModel=viewModel;this.inspector=inspector;this.coordinator=coordinator;this.createTelegramDialog=createTelegramDialog;DataContext=viewModel;backgroundRefresh.Tick+=(_,_)=>_=viewModel.RefreshStatesAsync(lifetime.Token);}
