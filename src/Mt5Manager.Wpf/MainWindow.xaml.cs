...
public partial class MainWindow:Window
{
 ...
 void Cleanup_Click(object sender,RoutedEventArgs e){if((sender as FrameworkElement)?.DataContext is TerminalRowViewModel row)new CleanupDialog(new CleanupViewModel(row.Terminal,inspector,coordinator,row.ApplyCleanupResult,row.RunStorageInspectionAsync)){Owner=this}.ShowDialog();}
}
