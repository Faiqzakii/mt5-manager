...
public partial class App:System.Windows.Application
{
 ...
 protected override void OnStartup(StartupEventArgs e){base.OnStartup(e);singleInstance=new Mutex(initiallyOwned:true,@"Local\Mt5Manager.SingleInstance",out var isFirstInstance);if(!isFirstInstance){MessageBox.Show("MT5 Manager is already running in this session.","MT5 Manager",MessageBoxButton.OK,MessageBoxImage.Information);singleInstance.Dispose();singleInstance=null;Shutdown();return;}var services=new ServiceCollection();
  ...
  services.AddSingleton<ITerminalDiscoverySource,ProcessDiscoverySource>();services.AddSingleton<ITerminalDiscoverySource,ShortcutDiscoverySource>();services.AddSingleton<ITerminalDiscoverySource,StandardLocationDiscoverySource>();services.AddSingleton<IMt5DataDirectoryResolver,Mt5DataDirectoryResolver>();services.AddSingleton<ITerminalDiscovery,TerminalDiscovery>();services.AddSingleton<TerminalOperationCoordinator>();services.AddSingleton<MainViewModel>(sp=>new MainViewModel(sp.GetRequiredService<ITerminalDiscovery>(),sp.GetRequiredService<ITerminalRegistry>(),sp.GetRequiredService<ITerminalProcessController>(),sp.GetRequiredService<ITerminalStorageInspector>(),sp.GetRequiredService<ITerminalRuntimeInspector>(),sp.GetRequiredService<ITerminalAlgoTradingController>()));services.AddSingleton<MainWindow>();provider=services.BuildServiceProvider();provider.GetRequiredService<MainWindow>().Show();}
